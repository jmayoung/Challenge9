...
const renderCurrentWeather = (currentWeather: any): void => {
  const { cityName, date, weatherIcon: iconCode, description, temperature, windSpeed, humidity } =
    currentWeather;
  ...
  tempEl.textContent = `Temp: ${((temperature - 273.15)* 1.8 + 32).toFixed(1)}°F`; 
...
const renderForecastCard = (forecast: any) => {
  const { date, weatherIcon: iconCode, description, temperature, windSpeed, humidity } = forecast;

  const { col, cardTitle, weatherIcon: iconEl, tempEl, windEl, humidityEl } = createForecastCard();
  ...
  if (iconCode) {
    ...
    console.error("Icon code is undefined for forecast:", forecast);
  }
